...
function loadJsonFile(){
	...
	//load skill
	$.getJSON( "line_json_file/skill.json", function( skill_row_data ) {
		skill_data = sortSkill(skill_row_data);
		initSkill(skill_data);
	});

	//load experience
	$.getJSON( "line_json_file/experience_data.json", function( experience_row_data ) {
	...
	$.each( skill_data , function( skill_type, skill_info_order ) {
		...
		$.each( skill_info_order , function( index, skill_info ) {
			$.each( skill_info , function( skill_name, skill ) {
				skill_dom += '		<div class="col-sm-3 col-md-2 col-lg-2 text-center no-padding-right">';
